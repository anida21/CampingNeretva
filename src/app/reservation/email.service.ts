...
export class EmailService {
  ...
  private baseUrl1 = 'http://localhost:3000'; // Promenite na odgovarajući URL vašeg Node.js servera
  ...
  sendRaftReservation(emailForm: EmailForm): Observable<any> {
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
    });

    return this.http.post<any>(`${this.baseUrl}/api/sendRaftReservation`, emailForm, { headers });
  }
